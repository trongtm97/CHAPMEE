...
export function MobileTopBar({ variant = "default" }: MobileTopBarProps) {
  ...
  const logoHeight = isCompact ? 26 : 30;

  return (
    <header
      className={`sticky top-0 z-40 isolate flex min-h-[3.25rem] shrink-0 items-center justify-between gap-2 border-b border-white/10 bg-[#0b1016]/90 px-3 backdrop-blur-2xl md:px-5 lg:hidden ${
        isCompact
          ? "pb-1.5 pt-[calc(env(safe-area-inset-top)+0.5rem)]"
    ...
    >
      ...
      <div className="flex min-w-0 items-center justify-end gap-1">
        <MobileTopBarSearch />
        <Link
          className="hidden min-h-9 shrink-0 items-center rounded-full border border-white/10 bg-white/[0.04] px-2 text-[0.65rem] font-bold text-zinc-400 transition hover:text-zinc-200 min-[400px]:inline-flex"
          href="/studio"
        >
          Studio
        </Link>
