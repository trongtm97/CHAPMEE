...
import { TrackedSwipeLink } from "@/components/swipe/TrackedSwipeLink";
...
type SwipeBottomOverlayProps = {
  context: SwipeAnalyticsContext;
};

export function SwipeBottomOverlay({ context }: SwipeBottomOverlayProps) {
  const item = context.item;
  const ctaExperiment = useExperiment("swipe_cta_copy");
  const ctaLabel =
    item.ctaLabel ||
  ...
  return (
    <aside className="pointer-events-none absolute inset-x-0 bottom-[calc(4.65rem+env(safe-area-inset-bottom))] z-20 px-4 sm:px-5">
      <div
        className="pointer-events-auto pr-[5.5rem] animate-[swipeOverlayIn_320ms_cubic-bezier(0.22,1,0.36,1)] sm:pr-[5.8rem]"
        key={item.id}
      >
