...
type SwipeCommentPanelProps = {
  context: SwipeAnalyticsContext | null;
  onOpenComments: () => void;
};

export function SwipeCommentPanel({ context, onOpenComments }: SwipeCommentPanelProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/swipe/comments?${query.toString()}`, { cache: "no-store" })
