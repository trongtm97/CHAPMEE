...
import { SwipeCommentInput } from "@/components/swipe/SwipeCommentInput";
import { SwipeCommentItem, type SwipeCommentView } from "@/components/swipe/SwipeCommentItem";
import { trackFeedComment, type SwipeAnalyticsContext } from "@/lib/analytics/trackSwipeEvents";

type SwipeCommentSheetProps = {
...
export function SwipeCommentSheet({
  context,
  onClose,
  onCommentCreated,
  open
}: SwipeCommentSheetProps) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/swipe/comments?${query.toString()}`, {
        cache: "no-store"
      });
  ...
  async function submitComment(content: string, parentId?: string) {
    ...
    try {
      const response = await fetch("/api/swipe/comments", {
        method: "POST",
        headers: {
  ...
  async function toggleCommentLike(commentId: string) {
    const response = await fetch(`/api/swipe/comments/${commentId}/like`, {
      method: "POST"
    });
  ...
  async function togglePin(commentId: string, pinned: boolean) {
    const response = await fetch(`/api/swipe/comments/${commentId}/pin`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
