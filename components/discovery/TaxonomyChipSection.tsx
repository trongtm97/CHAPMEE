...
export function TaxonomyChipSection({ section, className = "" }: TaxonomyChipSectionProps) {
  ...
  const terms = section.terms.slice(0, MAX_VISIBLE_CHIPS);
  ...
  return (
    <section className={`space-y-2 ${className}`.trim()}>
      <div className="flex items-center justify-between gap-2">
        ...
        <Link
          className="shrink-0 text-[11px] font-semibold text-cyan-200 hover:text-cyan-100"
      ...
      <div className="no-scrollbar -mx-4 overflow-x-auto px-4 md:mx-0 md:overflow-visible md:px-0">
        <div className="flex min-w-max gap-2 pb-0.5 md:min-w-0 md:flex-wrap">
          {terms.map((term) => (
            <Link
              className="tap-highlight whitespace-nowrap rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-semibold text-zinc-200 transition hover:border-cyan-300/35 hover:text-cyan-100"
              href={term.href}
              key={term.id}
            >
              {term.name}
