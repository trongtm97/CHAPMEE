...
import { loadChaptersForSwipeStoryAction } from "@/lib/swipe/swipe-actions";
import { SwipeBackgroundPicker } from "@/components/studio/swipe/SwipeBackgroundPicker";
import { SwipePreview } from "@/components/studio/swipe/SwipePreview";
import { SwipeSuggestionTools } from "@/components/studio/swipe/SwipeSuggestionTools";
...
import { SwipePublishChecklistPanel } from "@/components/studio/swipe/SwipePublishChecklistPanel";
import { validateSwipeBeforePublish } from "@/lib/publish/validate-swipe-before-publish";
import { autoTrimSwipeBody } from "@/lib/swipe/validate-swipe-item";
...
import {
  ...
  SWIPE_CTA_PRESETS,
  SWIPE_HOOK_MAX,
  type SwipeItemListItem,
  type SwipeSuggestionResult
} from "@/types/swipe";
...
export function SwipeEditor({
  ...
  useEffect(() => {
    ...
    void loadChaptersForSwipeStoryAction(storyId).then((result) => {
      if (!cancelled) {
  ...
  function applySuggestion(result: SwipeSuggestionResult) {
    setHook(result.hook);
    setBody(result.body);
  ...
  function handleCtaPresetChange(presetId: string) {
    setCtaType(presetId);
    const preset = SWIPE_CTA_PRESETS.find((item) => item.id === presetId);
  ...
  const swipeChecklistInput = useMemo(
    () => ({
      backgroundImageUrl,
      body,
  ...
  const publishRules = useMemo(
    () => validateSwipeBeforePublish(swipeChecklistInput).rules,
    [swipeChecklistInput]
  );
    ...
    startTransition(async () => {
      try {
        ...
        if (result && "ok" in result && result.ok === false) {
          setError(result.error ?? "Không thể lưu Swipe.");
          return;
        }
  ...
  const bodyTooLong = body.length > SWIPE_BODY_MAX;
  ...
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <form className="space-y-6" onSubmit={(event) => event.preventDefault()}>
        ...
        <section className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <h2 className="text-lg font-semibold text-white">Nội dung Swipe</h2>

          <SwipeSuggestionTools
          ...
          <Input
            ...
            value={hook}
          />
          <p className="-mt-3 text-xs text-zinc-500">{hook.length}/{SWIPE_HOOK_MAX} ký tự</p>

          <Textarea
            label="Nội dung"
            name="body_preview"
            onChange={(event) => setBody(event.target.value)}
            placeholder="Đoạn trích ngắn cho Swipe"
            required
            rows={8}
            value={body}
          />
          <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
            <span className={bodyTooLong ? "text-rose-300" : ""}>
              {body.length}/{SWIPE_BODY_MAX} ký tự (khuyến nghị 120–300)
            </span>
            {bodyTooLong ? (
              <Button
                onClick={() => setBody(autoTrimSwipeBody(body))}
                type="button"
                variant="secondary"
              >
                Cắt tự động
        ...
        <section className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          ...
          <SwipePublishChecklistPanel input={swipeChecklistInput} swipeId={swipeId} />
          {hasBlocking ? (
