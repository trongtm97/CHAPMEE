...
import {
  generateSwipeSuggestionFromStoryDescription,
  generateSwipeSuggestionsFromChapter
} from "@/lib/swipe/generate-swipe-suggestions";
import type { SwipeSourceType, SwipeSuggestionResult } from "@/types/swipe";

type SwipeSuggestionToolsProps = {
  chapterContent?: string | null;
  chapterTitle?: string | null;
  storyDescription?: string | null;
  storyTitle?: string;
  onApply: (result: SwipeSuggestionResult) => void;
};

export function SwipeSuggestionTools({
  chapterContent,
  chapterTitle,
  onApply,
  storyDescription,
  storyTitle
}: SwipeSuggestionToolsProps) {
  if (!chapterContent?.trim() && !storyDescription?.trim()) {
    return null;
  }

  function applyFromChapter(type: keyof ReturnType<typeof generateSwipeSuggestionsFromChapter>) {
    if (!chapterContent?.trim()) {
      return;
    }

    const suggestions = generateSwipeSuggestionsFromChapter({
      chapterTitle,
  ...
  function applyFromStoryDescription() {
    ...
    const result = generateSwipeSuggestionFromStoryDescription({
