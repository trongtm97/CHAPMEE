...
import type { ReelsItemListItem } from "@/types/reels";
...
type StudioReelsListCardProps = {
  item: ReelsItemListItem;
};

export function StudioReelsListCard({ item }: StudioReelsListCardProps) {
    ...
    <article className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:flex-row sm:items-start">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <Link
          ...
          href={studioPath(`/reels/${item.id}/edit`)}
        >
          Sửa
        </Link>
          ...
          {menuOpen ? (
            <div className="absolute right-0 z-10 mt-2 min-w-[10rem] rounded-xl border border-white/10 bg-zinc-950 p-1 shadow-lg">
              <Link
                ...
                href={studioPath(`/reels/${item.id}/edit?preview=1`)}
              ...
              <button
                ...
                onClick={() => {
                  setMenuOpen(false);
                  runAction(() => duplicateReelsItemAction(item.id));
                }}
                type="button"
              >
                Nhân bản
              ...
              {item.status === "published" ? (
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    runAction(() => hideReelsItemAction(item.id));
                  }}
                  type="button"
                >
                  Ẩn
              ...
              {item.status === "draft" ? (
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    runAction(() => deleteReelsDraftAction(item.id));
                  }}
                  type="button"
                >
                  Xóa nháp
