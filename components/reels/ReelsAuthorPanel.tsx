...
type ReelsAuthorPanelProps = {
  item: ReelsItem;
  isFollowBusy?: boolean;
  onToggleFollow: () => void;
};

export function ReelsAuthorPanel({
  item,
  isFollowBusy = false,
  onToggleFollow
}: ReelsAuthorPanelProps) {
  const creatorName = item.creatorName ?? "Tác giả ChapMee";
  const creatorHref = getCreatorPublicHref({
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="mt-3 flex items-center gap-3">
        {creatorHref ? (
          <Link className="tap-highlight shrink-0" href={creatorHref}>
