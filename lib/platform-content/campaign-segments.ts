...
async function getCreatorUserIds(activeOnly = false) {
  ...
  let query = db.from("creator_profiles").select("user_id");
  if (activeOnly) {
...
async function resolveSegmentUserIds(segment: NotificationCampaignSegment): Promise<Set<string>> {
  ...
  if (segment === "creators_with_story") {
    const { data: stories } = await db.from("stories").select("creator_id");
  ...
  if (segment === "verified_users") {
    const { data } = await db.from("profiles").select("id").eq("is_verified", true);
    for (const row of data ?? []) {
  ...
  if (segment === "unverified_users") {
    const { data } = await db.from("profiles").select("id").eq("is_verified", false);
    for (const row of data ?? []) {
      result.add(String(row.id));
    }
    return result;
