...
export async function fetchPaymentStatusSummary(range: FinanceDateRange) {
  ...
  let checkoutQuery = db.from("checkout_sessions").select("status", { count: "exact" });
...
export async function fetchWalletTotalsSnapshot() {
  ...
  const [creatorWallets, userWallets] = await Promise.all([
    db.from("creator_wallets").select("pending_revenue_vnd, available_revenue_vnd, locked_revenue_vnd"),
    db.from("user_wallets").select("paid_coin_balance, bonus_coin_balance")
  ]);
...
export async function countTransactionsForFinance(range: FinanceDateRange) {
  const db = await createClient();
  let query = db.from("transactions").select("id", { count: "exact", head: true });
  if (range.from) query = query.gte("created_at", range.from);
  if (range.to) query = query.lte("created_at", range.to);
