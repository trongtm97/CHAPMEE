...
export async function awardBadge(input: {
  ...
  const { data: badgeRow, error: badgeError } = await db
    .from("badges")
  ...
  if (error) {
    ...
    const { data: fallbackData, error: fallbackError } = await db
      .from("user_badges")
      .insert(payload)
      .select("id, awarded_at, related_story_id, metadata, badge_id")
      .maybeSingle();
    ...
    const fallbackRecord = fallbackData
      ? toBadgeRecord({ ...(fallbackData as UserBadgeRow), badges: badgeRow as BadgeRow })
      : null;

    return {
      alreadyAwarded: false,
      awarded: Boolean(fallbackRecord),
      badge: fallbackRecord
    };
  }
...
export async function getUserBadges(input: {
  ...
  if (error) {
    ...
    const rows = (fallback.data ?? []) as UserBadgeRow[];
    const badgeIds = [
      ...new Set(rows.map((row) => row.badge_id).filter((id): id is string => Boolean(id)))
    ];
    ...
    return rows
      .map((row) =>
        toBadgeRecord({
          ...row,
          badges: row.badge_id ? badgeById.get(row.badge_id) ?? null : null
        })
      )
