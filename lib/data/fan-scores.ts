...
async function recordScopeFanScore(input: {
  ...
  const { data, error } = await db.rpc("record_fan_score_event", {
...
export async function recordFanScoreAction(input: RecordFanScoreInput) {
  ...
  if (input.storyId) {
    ...
    const [topFansResult, storyResult] = await Promise.all([
      ...
      db.from("stories").select("id, title, slug, public_code").eq("id", input.storyId).maybeSingle()
    ]);
...
export async function getStoryTopFans(
  ...
  const db = await createClient();
  const { data, error } = await db.rpc("get_story_top_fans", {
    input_limit: limit,
...
export async function getUserTopFanHighlights(
  ...
  const db = await createClient();
  const { data, error } = await db.rpc("get_user_top_fan_highlights", {
    input_limit: limit,
