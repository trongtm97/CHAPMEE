...
async function resolveStoryIdsForSearch(supabase: Awaited<ReturnType<typeof createClient>>, q: string) {
...
async function resolveSpecialStatusIds(
  ...
  if (status === "partial_refund") {
    const { data } = await supabase.from("refunds").select("original_transaction_id").limit(1000);
    const ids = [
      ...new Set(
        (data ?? [])
          .map((row) => row.original_transaction_id as string | null)
...
export async function getTransactionsForAdminPaginated(
  ...
  if (options.search?.trim()) {
    ...
    const [{ data: profiles }, foundStoryIds] = await Promise.all([
      supabase
        ...
        .limit(20),
      resolveStoryIdsForSearch(supabase, q)
    ]);
  ...
  if (options.types?.includes("chargeback")) {
    const chargebackIds = await resolveSpecialStatusIds(supabase, "chargeback");
  ...
  if (options.status && options.status !== "all") {
    const specialIds = await resolveSpecialStatusIds(supabase, options.status);
    if (specialIds) {
