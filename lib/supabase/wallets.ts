...
export async function getOrCreateCreatorWalletRecord(userId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("ensure_creator_wallet", {
    input_user_id: userId
  });
...
export async function applyUserCoinLedgerRecord(input: {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("apply_user_coin_ledger", {
...
export async function applyCreatorRevenueLedgerRecord(input: {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("apply_creator_revenue_ledger", {
