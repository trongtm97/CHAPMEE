import {
  cleanSwipeSourceText,
  truncateSwipeBodyAtBoundary
} from "@/lib/swipe/clean-swipe-source-text";

const SNIPPET_MIN = 150;
const SNIPPET_MAX = 300;

function sliceSnippet(text: string, start: number, length = SNIPPET_MAX) {
  const normalized = cleanSwipeSourceText(text);
  ...
  if (snippet.length > SNIPPET_MAX) {
    snippet = truncateSwipeBodyAtBoundary(snippet, SNIPPET_MAX);
  }
...
export function extractDialogueSnippet(content: string) {
  const cleaned = cleanSwipeSourceText(content);
...
export function extractQuestionSnippet(content: string) {
  ...
  return {
    ...
    start,
    text: text.length > SNIPPET_MAX ? truncateSwipeBodyAtBoundary(text, SNIPPET_MAX) : text
  };
}
...
export function extractStoryDescriptionSnippet(description: string) {
  const cleaned = cleanSwipeSourceText(description);
