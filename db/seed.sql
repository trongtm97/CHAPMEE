-- ChapMee demo content seed (stories, episodes, taxonomy links, analytics).
--
-- Recommended (self-hosted local):
--   npm run db:setup
--   npm run db:seed
--
-- This file does NOT create auth users. `npm run db:seed` creates demo accounts
-- (reader/creator/admin @chapchap.local) and then runs this SQL.
--
-- Manual / legacy: requires at least one row in creator_profiles, then run in psql.
...
with selected_creator as (
  ...
  from public.creator_profiles cp
  join public.profiles p on p.id = cp.user_id
  order by case when p.username = 'demo_creator' then 0 else 1 end, cp.created_at, cp.id
  limit 1
),
story_seed (
  id,
...
select
  ...
  seed.title,
  seed.slug,
  '5' || right(split_part(seed.id::text, '-', 5), 9),
  seed.hook,
...
on conflict (slug) do update set
  ...
  public_code = coalesce(public.stories.public_code, excluded.public_code),
...
select
  ...
  seed.episode_number,
  seed.title,
  stories.slug || '-chuong-' || seed.episode_number::text,
  '6' || right(split_part(seed.id::text, '-', 5), 9),
  seed.content,
  seed.excerpt,
  seed.word_count,
  seed.status,
...
on conflict (story_id, episode_number) do update set
  title = excluded.title,
  slug = excluded.slug,
  public_code = coalesce(public.episodes.public_code, excluded.public_code),
