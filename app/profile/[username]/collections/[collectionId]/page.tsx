...
export async function generateMetadata({ params }: CollectionPageProps): Promise<Metadata> {
  ...
  const result = await getPublicCollectionForProfile(collectionId, username);
  if (!result) {
...
export default async function PublicProfileCollectionPage({ params }: CollectionPageProps) {
  ...
  const returnTo = getShareUrl(`/profile/${username}/collections/${collectionId}`);
  const isOwner = currentProfile?.id === result.owner.id;

  return (
    <div className="space-y-6 pb-8">
      <MobileBackHeader
        fallbackHref={`/profile/${username}`}
      ...
      <Card className="space-y-3 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <Link className="text-sm font-bold text-white" href={`/profile/${username}`}>
      ...
      {result.collection.items.length === 0 ? (
        <EmptyState
          description="Chủ tủ chưa thêm truyện nào."
          title="Tủ trống"
        />
      ) : (
        <div className="space-y-3">
          {result.collection.items.map((item, index) => (
