...
import { calculateNameDobReading } from "@/lib/love-insight/love-engine/scoring";
import type { LoveInput } from "@/lib/love-insight/love-engine/types";
import { parseNameDobReading, type NameDobReadingRequest } from "@/lib/love-insight/validators/love";

export const runtime = "nodejs";

const ROUTE = "POST:/api/v1/love/name-dob";
...
export async function POST(req: Request) {
  return withErrorHandling(ROUTE, async () => {
    ...
    const validation = parseNameDobReading(parsed.data);
    if (!validation.ok) {
      return jsonResponse(
        {
          ok: false,
    ...
    const result = calculateNameDobReading(input, dbData);
...
function toEngineInput(req: NameDobReadingRequest): LoveInput {
  return {
    personA: { name: req.personA.name, dob: req.personA.dob },
    personB: { name: req.personB.name, dob: req.personB.dob },
